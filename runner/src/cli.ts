#!/usr/bin/env node
/**
 * `rehearse` CLI — local-first GitHub Actions runner.
 */
...
// Read the version from our own package.json at runtime so `rehearse --version`
...
# Auto-generated by 'rehearse install-hook'. Edit or delete to disable.
exec npx -p @rehearse/runner rehearse run ${JSON.stringify(opts.workflow)}${opts.job ? ` --job ${JSON.stringify(opts.job)}` : ''} --fail-fast
`;
